    ...
    <div style="margin-bottom:10px">
        <table class="ui celled table compact unstackable two linebreak">
            ...
            <tr>
                ...
                <td>
                    <div class="ui form">
                        <form:select path="targetUserid" disabled="true" items="${persons}"/>
                    </div>
                </td>
                <th>수/발</th>
                    ...
                    <c:choose>
                        <c:when test="${files.size() > 0}">
                            <c:forEach var="e" items="${files}">
                                <audio controls src="${apiServerUrl}/api/v1/admin/record/history/resource?path=${g.urlEncode(e.filePath)}&token=${accessToken}" class="audio"></audio>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
                            녹취파일이 존재하지 않습니다.
