...
<form:form id="cdr-evaluation-form" modelAttribute="form" cssClass="-json-submit" data-method="post"
           action="${pageContext.request.contextPath}/api/evaluation-result/"
                    ...
                    <c:choose>
                        <c:when test="${files.size() > 0}">
                            <c:forEach var="e" items="${files}">
                                <audio controls src="${apiServerUrl}/api/v1/admin/record/history/resource?path=${g.urlEncode(e.filePath)}&mode=PLAY&token=${accessToken}" class="audio"></audio>
                            </c:forEach>
                        </c:when>
                        <c:otherwise>
                            녹취파일이 존재하지 않습니다.
            ...
            <tr>
                ...
                <td colspan="5" style="text-align: left;">
                    <div class="ui checkbox">
                        <form:checkbox path="resultTransfer" value="${true}"/>
                    </div>
                </td>
            </tr>
            </tbody>
