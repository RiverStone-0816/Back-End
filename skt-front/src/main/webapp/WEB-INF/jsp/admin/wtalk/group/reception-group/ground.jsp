...
<tags:tabContentLayout>
    <div class="content-wrapper-frame">
        <tags:page-menu-tab url="/admin/wtalk/group/reception-group/"/>
                                    ...
                                    <tr data-id="${e.groupId}">
                                        ...
                                        <td title="<c:forEach var="person" items="${e.persons}">[${g.htmlQuote(person.idName)}(${g.htmlQuote(person.id)})]</c:forEach>">
                                            <c:forEach var="person" items="${e.persons}">
                                                <c:if test="${e.talkStrategy == 'RR'}">[순차${person.distributionSequence + 1}]</c:if>
                                                [${g.htmlQuote(person.idName)}(${g.htmlQuote(person.id)})]
                                            </c:forEach>
                                        </td>
                                    </tr>
                                </c:forEach>
    ...
    <tags:scripts>
        <script>
            function popupModal(groupId) {
                popupReceivedHtml('/admin/wtalk/group/reception-group/' + (groupId || 'new') + '/modal', 'modal-talk-reception-group');
            }

            function deleteEntity(groupId) {
                confirm('정말 삭제하시겠습니까?').done(function () {
                    restSelf.delete('/api/wtalk-reception-group/' + groupId).done(function () {
                        reload();
                    });
                });
            }
