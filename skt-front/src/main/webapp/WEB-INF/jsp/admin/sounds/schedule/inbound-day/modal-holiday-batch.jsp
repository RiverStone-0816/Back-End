...
<form:form modelAttribute="form" class="ui modal -json-submit" data-method="post"
           action="${pageContext.request.contextPath}/api/inbound-day-schedule/holy"
                        ...
                        <div class="from-panel">
                            <select class="form-control -right-selector" size="8" multiple="multiple">
                                <c:forEach var="e" items="${number070List}">
                                    <option value="${g.htmlQuote(e.number)}">[${g.htmlQuote(g.messageOf('NumberType', e.type))}]-${g.htmlQuote(e.number)}[${g.htmlQuote(e.hanName != null ? e.hanName : '')}]</option>
                                </c:forEach>
                            </select>
                        </div>
