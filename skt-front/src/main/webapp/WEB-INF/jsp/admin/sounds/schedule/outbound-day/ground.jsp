...
<tags:tabContentLayout>
    <div class="content-wrapper-frame">
        <tags:page-menu-tab url="/admin/sounds/schedule/outbound-day/"/>
        <div class="sub-content ui container fluid unstackable">
            <form:form id="search-form" modelAttribute="search" method="get" class="panel panel-search">
                <div class="panel-heading">
                    <div class="pull-left">
                        <div class="panel-label">${g.htmlQuote(menu.getMenuName("/admin/sounds/schedule/outbound-day/"))}</div>
                    </div>
                                        ...
                                        <c:when test="${e.outScheduleLists != null && e.outScheduleLists.size() > 0}">
                                            <c:forEach var="scheduleListItem" items="${e.outScheduleLists}" varStatus="infoStatus">
                                                <tr>
                                                    ...
                                                    <td>
                                                        <c:choose>
                                                            <c:when test="${g.dateFormat(scheduleListItem.fromtime) == g.dateFormat(scheduleListItem.totime)}">
                                                                ${g.dateFormat(scheduleListItem.fromtime)}
                                                            </c:when>
                                                            <c:otherwise>
                                                                ${g.dateFormat(scheduleListItem.fromtime)} ~ ${g.dateFormat(scheduleListItem.totime)}
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </td>
                                                    <td>
    ...
    <tags:scripts>
        <script>
            function popupHolidayBatchModal() {
                popupReceivedHtml('/admin/sounds/schedule/outbound-day/modal-holiday-batch', 'modal-holiday-batch');
            }
            ...
            function deleteEntity(parent) {
                confirm('정말 삭제하시겠습니까?').done(function () {
                    restSelf.delete('/api/outbound-day-schedule/' + parent).done(function () {
                        reload();
                    });
                });
            }
