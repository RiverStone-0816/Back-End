...
<form:form modelAttribute="form" cssClass="ui modal large -json-submit" data-method="put"
    ...
    <div class="header">유형수정[상담결과]</div>
            ...
            <div class="row">
                <table class="ui celled table compact unstackable fixed">
                    ...
                    <c:if test="${usingServices.contains('TYPE2')}">
                        <tbody>
                        <c:forEach var="field" items="${fields}" varStatus="fieldStatus">
                            <c:if test="${g.htmlQuote(fieldIdToNames.get(field.id))=='코드값_1'||
                            g.htmlQuote(fieldIdToNames.get(field.id))=='코드값_2'||
                            g.htmlQuote(fieldIdToNames.get(field.id))=='코드값_3'||
                            g.htmlQuote(fieldIdToNames.get(field.id))=='문자열_1'||
                            g.htmlQuote(fieldIdToNames.get(field.id))=='문자열_2'}">
                                <tr>
                                    <td>
                                        <input type="hidden" name="field[${fieldStatus.index}].id" value="${field.id}"/>
                                        <c:choose>
                                    ...
                                    <td>
                                        <div class="ui form fluid">
                                            <select name="field[${fieldStatus.index}].isdisplay">
                                                <option value="Y" ${field.isdisplay == 'Y' ? 'selected' : null}>YES
                                                </option>
