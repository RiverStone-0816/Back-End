...
<form:form modelAttribute="form" cssClass="ui modal -json-submit" data-method="post"
           action="${pageContext.request.contextPath}/api/manual/${entity == null ? null : entity.id}"
           data-before="prepareWriteFormData" data-done="reload">

    <i class="close icon"></i>
    <div class="header">메뉴얼[${entity != null ? '수정' : '추가'}]</div>
                        ...
                        <div class="ui list" data-count="${entity != null ? entity.fileInfo.size() : 0}">
                            <c:forEach var="file" items="${entity.fileInfo}">
                                <div class="item">
                                    ...
                                    <a target="_blank" href="${apiServerUrl}/api/v1/admin/help/manual/${file.id}/specific-file-resource?token=${accessToken}">${g.htmlQuote(file.originalName)}</a>
                                </div>
                            </c:forEach>
                        </div>
                    </div>
