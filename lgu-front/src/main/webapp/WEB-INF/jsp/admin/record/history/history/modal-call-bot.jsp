...
<script>
    ...
    const callBotRecordHistoryProperties = {
        ...
        data: function () {
            return {
                uniqueId: '${not empty cdr ? cdr.uniqueid : ''}',
                callStartTime: null,
                callEndTime: null,
                ani: '${not empty cdr ? (cdr.inOut eq 'I' ? cdr.src : cdr.dst ): ''}',
                dnis: '${not empty cdr ? (cdr.inOut eq 'I' ? cdr.dst : cdr.src ): ''}',
        ...
        methods: {
            ...
            loadRoom: function () {
                const _this = this
                <c:if test="${not empty cdr}">
                return restSelf.get('/api/record-history/call-bot/${cdr.inOut eq 'I' ? cdr.uniqueid : cdr.cmpClickKey}/' + ('${cdr.ringDate}'.split(' ')[0].replaceAll('-', '')) + '/${cdr.etc5}/${cdr.etc4}/${cdr.inOut eq 'I' ? cdr.src : cdr.dst}').done(function (response) {
