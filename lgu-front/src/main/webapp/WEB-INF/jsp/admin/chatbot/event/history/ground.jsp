...
<tags:tabContentLayout>
    <div class="content-wrapper-frame">
        <tags:page-menu-tab url="/admin/chatbot/event/history/"/>
        <div class="sub-content ui container fluid">
            <form:form id="search-form" modelAttribute="search" method="get" class="panel panel-search">
                ...
                <div class="panel-body">
                    <div class="search-area">
                        <div class="ui grid">
                            <div class="row">
                                <div class="two wide column"><label class="control-label">전송날짜</label></div>
                            ...
                            <div class="row">
                                <div class="two wide column"><label class="control-label">이벤트선택</label></div>
                                <div class="three wide column">
                                    <div class="ui form">
                                        <select id="eventSelector">
                                            <option value="">선택안함</option>
                                            <c:forEach var="e" items="${chatbotEventList}">
                                                <option data-chatbot-id="${e.chatbotId}" data-event-name="${e.eventName}"  ${e.chatbotId == search.botId && e.eventName == search.eventName ? 'selected' : ''}>
                                                    [${g.htmlQuote(e.chatbotName)}][${g.htmlQuote(e.blockName)}][${g.htmlQuote(e.eventName)}]
                                                </option>
                                            </c:forEach>
                                        </select>
            ...
            <div class="panel">
                            ...
                            <c:when test="${pagination.rows.size() > 0}">
                                <c:forEach var="e" items="${pagination.rows}" varStatus="status">
                                    <tr>
                                        ...
                                        <td><fmt:formatDate value="${e.sendTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                        <td>${g.htmlQuote(e.chatbotName)}</td>
                                        <td>${g.htmlQuote(e.eventName)}</td>
                                        <td>${g.htmlQuote(e.profileName)}</td>
                                        <td>${g.htmlQuote(e.eventData)}</td>
                                        <td>${g.htmlQuote(e.blockName)}</td>
                                        <td>${g.htmlQuote(e.isReceive)}</td>
                                    </tr>
                                </c:forEach>
                            </c:when>
                            <c:otherwise>
                ...
                <div class="panel-footer">
                    <tags:pagination navigation="${pagination.navigation}" url="${pageContext.request.contextPath}/admin/chatbot/event/history" pageForm="${search}"/>
                </div>
            </div>
        </div>
    </div>
    <tags:scripts>
        <script>
            $('#eventSelector').on('change', function () {
                $('#botId').val($(this).find('option:selected').data('chatbot-id'));
                $('#eventName').val($(this).find('option:selected').data('event-name'));
            });
        </script>
    </tags:scripts>
</tags:tabContentLayout>