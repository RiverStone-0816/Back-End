...
<form:form commandName="form" cssClass="ui modal small -json-submit" data-method="${entity != null ? 'put' : 'post'}"
           action="${pageContext.request.contextPath}/api/pds-custominfo/${entity != null ? g.htmlQuote(entity.pdsSysCustomId) : null}"
           data-before="prepareWriteFormData" data-done="reload">

    <i class="close icon"></i>
    <div class="header">고객DB${entity != null ? '수정' : '추가'}[그룹명:${g.htmlQuote(group.name)}]</div>
...
<script>
    ...
    const pdsCustomInfo = $('#modal-data');

    const DBTYPE_FIELD_PREFIX = 'PDS_';
    ...
    pdsCustomInfo.find('select').change(function () {
        ...
        const relatedField = pdsCustomInfo.find('[name="' + convertToFormFieldId(fieldToRelatedField[dbName]) + '"]');
