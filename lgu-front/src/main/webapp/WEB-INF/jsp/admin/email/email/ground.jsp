...
<tags:tabContentLayout>
    <div class="content-wrapper-frame">
        <tags:page-menu-tab url="/admin/email/email/"/>
                            ...
                            <c:when test="${pagination.rows.size() > 0}">
                                <c:forEach var="e" items="${pagination.rows}" varStatus="status">
                                    <tr data-id="${e.seq}">
                                        ...
                                        <td>${g.htmlQuote(e.mailUserName)}</td>
                                        <td>${g.htmlQuote(e.mailProtocol)}</td>
                                        <td>${g.htmlQuote(e.sendEmail)}</td>
                                        <td>${g.htmlQuote(e.sendEmailName)}</td>
                                    </tr>
                                </c:forEach>
                            </c:when>
                            <c:otherwise>
    ...
    <tags:scripts>
        <script>
            function popupModal(seq) {
                popupReceivedHtml('/admin/email/email/' + (seq || 'new') + '/modal', 'modal-email');
            }

            function deleteEntity(seq) {
                confirm('정말 삭제하시겠습니까?').done(function () {
                    restSelf.delete('/api/email/' + seq).done(function () {
                        reload();
                    });
                });
            }
