    ...
    <div class="panel-body">
        <div class="search-area">
            <div class="ui grid">
                ...
                <div class="row">
                    <div class="three wide column"><label class="control-label">부서선택</label></div>
...
<div class="panel">
    ...
    <div class="panel-body" style="width: 100%; overflow-x: auto;">
        <table class="ui celled table structured compact unstackable ${pagination.rows.size() > 0 ? "selectable-only" : null}"
               data-entity="RecordHistory">
            <thead>
            <tr>
                <th>번호</th>
                <th>대표서비스</th>
            ...
            <c:choose>
                <c:when test="${pagination.rows.size() > 0}">
                    <c:forEach var="e" items="${pagination.rows}" varStatus="status">
                        <c:set var="isFile"
                               value="${g.htmlQuote(e.callStatusValue.contains('정상통화')) and fn:length(g.htmlQuote(e.recordFile)) > 0 && e.recordInfo != 'S'}"/>
                        <tr data-id="${e.seq}" data-phone-number="${e.inOut.contains('I') ? e.src : e.dst}"
                            ...
                            <td>
                                <c:set var="userName"
                                       value="${e.personList != null && e.personList.idName != null && e.personList.idName.length() > 0 ? e.personList.idName : null}"/>
                                <c:choose>
