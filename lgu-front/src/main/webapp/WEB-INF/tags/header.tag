        ...
        <div class="pull-left" id="call-control-panel" style="display: none;">
            <div class="gnb-btn-wrap" id="user-ivr">
                ...
                <c:if test="${g.usingServices.contains('SMS')}">
                    <button class="ui button basic compact white" onclick="smsSend()"><i class="material-icons"> sms </i>SMS발송</button>
                </c:if>
                ...
                <div class="selectbox header-nav-container">
                    ...
                    <div class="option nav-ul">
                        <ul>
                            ...
                            <c:if test="${
                            'J|A'.contains(g.user.idType) and
                            (
                                g.usingServices.contains('ASTIN') or
                                g.usingServices.contains('ASTOUT') or
                                g.usingServices.contains('BSTT')
                            )
                            }">
                                <li><a class="condition-01" href="javascript:" onclick="assistOutOpen();">상담지원</a></li>
                            </c:if>

                            <%-- 상담사 이면서 ASTOUT 라이센스 가지고 있음. (B조건) --%>
                            <c:if test="${
                            'M'.contains(g.user.idType) and
                            g.usingServices.contains('ASTOUT') and
                            g.user.isAstOut == 'Y'
                            }">
                                <li class="condition-02"><a href="javascript:" onclick="assistOutOpen();">상담지원</a></li>
                            </c:if>
...
<tags:scripts>
    <script>
        const ssoRequestSiteUrl = "${ssoRequestSiteUrl}"
        const sttSocketUrl = "${sttSocketUrl}"

        function assistOutOpen() {
            // SSO 처리
            restSelf.get('/counsel/assist-sso').done((res) => {
                console.log('res = ', res)
                const {result, result_data} = res.data
                if(result === 'OK' && result_data){
                    const company_id = '${g.getUser().getCompanyId()}';
                    console.log('SSO 성공 company_id = ', company_id)
                    console.log('SSO 성공 result_data = ', result_data)

                    console.log('ssoRequestSiteUrl = ', ssoRequestSiteUrl)
                    window.open(ssoRequestSiteUrl + '?company_id=' + company_id + '&' + 'token=' + result_data);
                }else{
                    alert("SSO 처리에 오류가 발생 했습니다.")
                    console.log('result = ', result, 'result_data = ', result_data)
                }
            })
        }
        ...
        function changeMode() {
            ...
            if ($('#main').is('.change-mode')) {
                ...
                $('#assist-custom-sidebar').show();
            } else {
                $('.tab-menu').contents().find('.excel-down-button').show();
                $('#assist-custom-sidebar').hide();
            }

            // 상담사 화면인 경우에 assistSocket 사용
            if($('main#main').hasClass('change-mode')) {
                <c:if test="${g.usingServices.contains('ASTIN') && g.user.isAstIn eq 'Y'}">
                    initAssistSocketReady(sttSocketUrl)
                </c:if>
                loginStatus()
            }
        }
        ...
        (function () {
            if (${hasExtension && isCti && user.idType eq 'M'} || ${isTalk && user.idType eq 'M'})
                changeMode();
        })();
    </script>
</tags:scripts>
