...
const ipccCommunicator = new IpccCommunicator()
    ...
    .on('CALLEVENT', function (message, kind /*[ IR | ID | OR | OD | ... ]*/, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12) {
        ...
        if (kind === 'IR') { // 인바운드 링울림
            ...
            const callingPathText = (services[callingPath] != null ? '인입경로:' + services[callingPath]: callingPath)

            $(".-calling-path").val(callingPathText).text(callingPathText);
            $('.-calling-number').val(phoneNumber).text(phoneNumber.replace(/^(\d{2,3})(\d{3,4})(\d{4})$/, `$1-$2-$3`));
