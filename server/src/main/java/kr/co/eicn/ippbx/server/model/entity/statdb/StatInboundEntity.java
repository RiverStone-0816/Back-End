...
public class StatInboundEntity extends CommonStatInbound implements StatByTimeUnit {
    ...
    public Long getWaitAvg() {
        return getConnreq() == 0 ? 0L : getWaitSum() / getConnreq();
    }

    public Double getResponseRate() {
        return Double.parseDouble(String.format("%.2f", EicnUtils.getRateValue(getSuccess(), getTotal() - getOnlyread())));
    }
    ...
    public Integer getTotalCancel() {
        return (getCancel() == null ? 0 : getCancel()) +
                (getCancelTimeout() == null ? 0 : getCancelTimeout()) +
                (getCancelCallback() == null ? 0 : getCancelCallback()) +
                (getCancelCustom() == null ? 0 : getCancelCustom());
    }

    public Integer getSuccessSum() {
        return getSuccess() + getCallbackSuccess();
    }
