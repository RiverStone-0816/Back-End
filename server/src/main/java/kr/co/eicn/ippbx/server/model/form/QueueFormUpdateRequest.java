...
public class QueueFormUpdateRequest extends BaseForm {
	...
	private String retryAction;		//재시도 버튼
	@NotNull("비연결시 행동")
	...
	public boolean validate(BindingResult bindingResult) {
		...
		if (ForwardingType.INNER.getCode().equals(isForwarding) || ForwardingType.OUTER.getCode().equals(isForwarding))
			if (isEmpty(huntForwarding))
				reject(bindingResult, "huntForwarding", "validator.must.select", "내부 포워딩 및 외부 포워딩할 번호(를)");
		if (addPersons != null && addPersons.size() > 0)
			for (QueuePersonFormRequest addPerson : addPersons) {
				if (isEmpty(addPerson.getPeer())) {
					reject(bindingResult, "addPersons", "validator.must.select", "내선 전화기 ID");
					break;
				}
			}
		if (Boolean.TRUE.equals(isRetry)) {
			if (retryMaxCount != null && retryMaxCount > 0) {
				if (isEmpty(retrySoundCode))
					reject(bindingResult, "retrySoundCode", "validator.must.select", "재시도 음원");
				if (isEmpty(retryAction))
					reject(bindingResult, "retrySoundCode", "validator.must.select", "재시도 버튼");
			} else
				reject(bindingResult, "retryMaxCount", "validator.must.select", "재시도 횟수(를)");
		}
