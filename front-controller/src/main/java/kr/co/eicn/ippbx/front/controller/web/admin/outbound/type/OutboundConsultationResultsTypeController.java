...
import kr.co.eicn.ippbx.front.service.api.outbound.type.OutboundCommonTypeApiInterface;
...
@LoginRequired
@Controller
@RequestMapping("admin/outbound/type/consultation-results-type")
public class OutboundConsultationResultsTypeController extends BaseController {
    private static final Logger logger = LoggerFactory.getLogger(OutboundConsultationResultsTypeController.class);

    @Autowired
    private OutboundCommonTypeApiInterface apiInterface;

    @GetMapping("")
    public String page(Model model) throws IOException, ResultFailException {
        final List<CommonTypeEntity> list = apiInterface.list(CommonTypeKind.CONSULTATION_RESULTS.getCode()).stream().filter(e -> e.getType().equals("OUT")).collect(Collectors.toList());
        model.addAttribute("list", list);
        return "admin/outbound/type/consultation-results-type/ground";
    }
    ...
    public String modal(Model model, @ModelAttribute("form") CommonTypeUpdateFormRequest form, @PathVariable Integer seq) throws IOException, ResultFailException {
        ...
        return "admin/outbound/type/consultation-results-type/modal-update-type";
    }
    ...
    public String modal(Model model, @PathVariable Integer seq) throws IOException, ResultFailException {
        ...
        return "admin/outbound/type/consultation-results-type/modal-update-sequence-fields";
    }
}
