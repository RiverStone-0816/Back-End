...
@Service
public class OutboundCommonTypeApiInterface extends ApiServerInterface {
    private static final Logger logger = LoggerFactory.getLogger(OutboundCommonTypeApiInterface.class);
    private static final String subUrl = "/api/v1/admin/outbound/type/";

    public List<CommonTypeEntity> list(String kind) throws IOException, ResultFailException {
        return getList(subUrl, Collections.singletonMap("kind", kind), CommonTypeEntity.class).getData();
    }
    ...
    public List<CommonBasicField> getBasicFields(String kind) throws IOException, ResultFailException {
        return getList(subUrl + "basic-field", Collections.singletonMap("kind", kind), CommonBasicField.class).getData();
    }
}
