...
public class PdsResultController extends BaseController {
    ...
    public static Map<String, Object> createFieldNameToValueMap(ResultCustomInfo entity, CommonTypeEntity resultType) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        ...
        for (CommonFieldEntity field : resultType.getFields()) {
            ...
            fieldNameToValueMap.put(field.getFieldId(), ResultCustomInfo.class.getDeclaredMethod("getRs" + capName).invoke(entity));
        }
    ...
    public String page(Model model, @ModelAttribute("search") PdsResultSearch search) {
        val executingPdsList = apiInterface.getExecutingPdsList();
        ...
        model.addAttribute("executingPdsList", new MapToLinkedHashMap().toLinkedHashMapByValue(executingPdsList.stream().collect(Collectors.toMap(HistoryPdsGroupResponse::getExecuteId, HistoryPdsGroupResponse::getExecuteName))));
        ...
        final CommonTypeEntity resultType = commonTypeApiInterface.get(executingPds.getResultType());
        model.addAttribute("resultType", resultType);

        final Pagination<PDSResultCustomInfoEntity> pagination = apiInterface.getPagination(search.getExecuteId(), search.convertToRequest(""));
        model.addAttribute("pagination", pagination);

        model.addAttribute("customIdToFieldNameToValueMap", PdsCustominfoController.createCustomIdToFieldNameToValueMap(pagination.getRows().stream().map(PDSResultCustomInfoEntity::getCustomInfo).collect(Collectors.toList()), pdsType));
    ...
    public void downloadExcel(PdsResultSearch search, HttpServletResponse response) {
        ...
        new PdsResultExcel(pagination.getRows(), pdsType, resultType).generator(response, "PDS결과이력");
    }
}
