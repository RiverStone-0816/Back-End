...
public class TalkTemplateRepository extends EicnBaseRepository<TalkTemplate, TalkTemplateEntity, Integer> {
    ...
    public Integer insert(TalkTemplateFormRequest form) {
        return dsl.insertInto(TALK_TEMPLATE)
                .set(TALK_TEMPLATE.TYPE, form.getType())
                .set(TALK_TEMPLATE.TYPE_MENT, form.getTypeMent().getCode())
                .set(TALK_TEMPLATE.TYPE_DATA, form.getTypeData())
                .set(TALK_TEMPLATE.MENT_NAME, form.getMentName())
                .set(TALK_TEMPLATE.MENT, form.getMent())
                .set(TALK_TEMPLATE.ORIGINAL_FILE_NAME, form.getOriginalFileName())
                .set(TALK_TEMPLATE.FILE_PATH, form.getFilePath())
                .set(TALK_TEMPLATE.WRITE_USERID, g.getUser().getId())
                .set(TALK_TEMPLATE.COMPANY_ID, g.getUser().getCompanyId())
                .returning(TALK_TEMPLATE.SEQ)
                .fetchOne()
                .get(TALK_TEMPLATE.SEQ);
    }

    public void update(Integer seq, TalkTemplateFormRequest form) {
        dsl.update(TALK_TEMPLATE)
                .set(TALK_TEMPLATE.TYPE, form.getType())
                .set(TALK_TEMPLATE.TYPE_MENT, form.getTypeMent().getCode())
                .set(TALK_TEMPLATE.TYPE_DATA, form.getTypeData())
                .set(TALK_TEMPLATE.MENT_NAME, form.getMentName())
                ...
                .set(TALK_TEMPLATE.ORIGINAL_FILE_NAME, form.getOriginalFileName())
                .set(TALK_TEMPLATE.FILE_PATH, form.getFilePath())
                .where(TALK_TEMPLATE.SEQ.eq(seq))
                .execute();
    }
}
