...
@RestController
@RequestMapping(value = "api/v1/admin/outbound/code", produces = MediaType.APPLICATION_JSON_VALUE)
public class OutboundCommonCodeApiController extends ApiBaseController {
    ...
    public ResponseEntity<JsonResult<List<CommonTypeDetailResponse>>> list(@RequestParam (required = false) String kind) {
        ...
        List<CommonType> commonTypeList = commonTypeRepository.findAllByKind(kind);
     ...
     *  코드 엑셀업로드
     * */
    @PostMapping(value = "type/{type}/field/{fieldId}/codes/by-excel", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
