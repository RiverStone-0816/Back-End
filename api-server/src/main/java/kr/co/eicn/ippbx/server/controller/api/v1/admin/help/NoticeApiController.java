...
import kr.co.eicn.ippbx.server.model.dto.eicn.NoticeDetailResponse;
...
import kr.co.eicn.ippbx.server.service.NoticeFileUploadService;
...
@RestController
@RequestMapping(value = "api/v1/admin/help/notice", produces = MediaType.APPLICATION_JSON_VALUE)
public class NoticeApiController extends ApiBaseController {

    private final NoticeRepository repository;
    private final NoticeFileEntityRepository fileEntityRepository;
    private final NoticeXFileRepository xFileRepository;
    private final PersonListRepository personListRepository;
    private final NoticeFileUploadService service;
    ...
    public ResponseEntity<JsonResult<Void>> post(@Valid NoticeFormRequest form, BindingResult bindingResult) {
        if (!form.validate(bindingResult))
            throw new ValidationException(bindingResult);

        if (form.getFiles() != null) {
            service.insertNoticeWithFileStore(form);
        } else {
            repository.insertOnGeneratedKey(form);
        }
        return ResponseEntity.created(URI.create("api/v1/admin/help/notice")).body(create());
    }

    @PostMapping(value = "{id}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<JsonResult<Void>> put(@Valid NoticeFormRequest form, BindingResult bindingResult,
    ...
    public ResponseEntity<JsonResult<Void>> delete(@PathVariable Long id) {
        service.deleteNoticeWithFileStore(id);
        return ResponseEntity.ok(create());
    }
     ...
     * 특정 파일 삭제
     */
    @DeleteMapping(value = "delete-specific-file/{fileId}")
    public ResponseEntity<JsonResult<Void>> deleteSpecificFile(@PathVariable Long fileId) {
        service.deleteSpecificFile(fileId);
        return ResponseEntity.ok(create());
    }
    ...
    public ResponseEntity<Resource> specificFileResource(@PathVariable Long fileId, HttpServletRequest request) {
        final NoticeFileEntity entity = service.findOneIfNullThrow(fileId);
