...
public class AESFileEncrypt256 {
	...
	public AESFileEncrypt256(String keyStr) throws NoSuchAlgorithmException, InvalidKeyException, InvalidKeySpecException {
		try {
			byte[] iv = INIT_VECTOR.getBytes("UTF-8");
	...
	* @return
	*/
	public static Key generateKey(String keyStr) throws NoSuchAlgorithmException, InvalidKeyException, InvalidKeySpecException {
		SecretKeySpec keySpec = null;

		try {
			// Key size 맞춤 (256bit, 32byte)
			byte[] key = Arrays.copyOf( (keyStr+KEY_PADDING).getBytes("UTF-8"), AES_KEY_SIZE_256 / 8 );
