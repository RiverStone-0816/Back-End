...
public class PDSGroupApiController extends ApiBaseController {
    ...
    public ResponseEntity<JsonResult<List<PDSGroupSummaryResponse>>> list(PDSGroupSearchRequest search) {
        ...
        return ResponseEntity.ok(data(repository.findAll(search).stream().map((e) -> convertDto(e, PDSGroupSummaryResponse.class)).toList()));
    }
    ...
    public ResponseEntity<JsonResult<Pagination<PDSGroupSummaryResponse>>> pagination(PDSGroupSearchRequest search) {
        ...
        final Map<Integer, String> pdsTypeNameMap = commonTypeRepository.findByKindStatus(CommonTypeKind.PDS, CommonTypeStatus.USING).stream().collect(Collectors.toMap(CommonType::getSeq, CommonType::getName));
        ...
        final List<PDSGroupSummaryResponse> rows = pagination.getRows().stream()
                .map(e -> {
                    final PDSGroupSummaryResponse response = convertDto(e, PDSGroupSummaryResponse.class);
                    response.setPdsTypeValue(pdsTypeNameMap.getOrDefault(e.getPdsType(), ""));
                    response.setConnectDataValue(connectDataValueMap.containsKey(e.getConnectKind()) ? connectDataValueMap.get(e.getConnectKind()).getOrDefault(e.getConnectData(), "") : "");
    ...
    public ResponseEntity<JsonResult<List<SummaryCommonTypeResponse>>> addCommonTypeLists(@RequestParam String kind) {
        ...
        return ResponseEntity.ok(data(
                commonTypeRepository.findByKindStatus(enumKind, CommonTypeStatus.USING)
                        .stream()
    ...
    public ResponseEntity<JsonResult<List<SummaryNumber070Response>>> addNumberLists() {
        return ResponseEntity.ok(data(
                numberRepository.findAll(NUMBER_070.NUMBER.like("070%"))
                        .stream()
    ...
    public ResponseEntity<JsonResult<List<SummaryPDSQueueNameResponse>>> addPDSQueueNameLists() {
        return ResponseEntity.ok(data(
                pdsQueueNameRepository.findAll()
                        .stream()
    ...
    public ResponseEntity<JsonResult<List<SummaryIvrTreeResponse>>> addPDSIvrLists() {
        return ResponseEntity.ok(data(
                pdsIvrRepository.findAll(PDS_IVR.PARENT.eq(0))
                        .stream()
    ...
    public ResponseEntity<JsonResult<List<SummaryCommonTypeResponse>>> addConsultationResult() {
        return ResponseEntity.ok(data(
                commonTypeRepository.findByKindStatus(CommonTypeKind.CONSULTATION_RESULTS, CommonTypeStatus.USING)
                        .stream()
                        .map(e -> convertDto(e, SummaryCommonTypeResponse.class))
                        .collect(Collectors.toList()))
        );
    ...
    public ResponseEntity<JsonResult<List<SummaryResearchListResponse>>> addResearchLists() {
        return ResponseEntity.ok(data(
                researchListRepository.findAll()
                        .stream()
    ...
    public ResponseEntity<JsonResult<List<SummaryCommonFieldResponse>>> addCommonFieldLists() {
        return ResponseEntity.ok(data(
                commonFieldRepository.findAll(COMMON_FIELD.FIELD_TYPE.eq("NUMBER"))
                        .stream()
