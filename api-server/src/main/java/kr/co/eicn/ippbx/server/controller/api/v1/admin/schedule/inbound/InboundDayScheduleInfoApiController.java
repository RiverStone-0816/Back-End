...
@RestController
@RequestMapping(value = "api/v1/admin/sounds/schedule/inbound/day", produces = MediaType.APPLICATION_JSON_VALUE)
public class InboundDayScheduleInfoApiController extends ApiBaseController {
	 ...
	 */
	@PostMapping("")
	public ResponseEntity<JsonResult<Void>> post(@Valid @RequestBody DayScheduleInfoFormRequest form, BindingResult bindingResult) {
		if (!form.validate(bindingResult))
			throw new ValidationException(bindingResult);

		repository.insertOnDaySchedule(form);
		return ResponseEntity.created(URI.create("api/v1/admin/sounds/schedule/inbound/day")).body(create());
	...
	public ResponseEntity<JsonResult<Void>> delete(@PathVariable String number) {
		repository.deleteByNumber(ScheduleType.DAY, number);
	...
	public ResponseEntity<JsonResult<Void>> updateType(@Valid @RequestBody DayScheduleInfoUpdateFormRequest form, BindingResult bindingResult, @PathVariable Integer seq) {
		if (!form.validate(bindingResult))
			throw new ValidationException(bindingResult);

		repository.updateByDayType(form, seq);
		return ResponseEntity.ok(create());
	}
	...
	public ResponseEntity<JsonResult<Void>> deleteType(@PathVariable Integer seq) {
		repository.deleteAllPBXServer(seq);
		return ResponseEntity.ok(create());
	}
	 ...
	 */
	@PostMapping("holy")
	public ResponseEntity<JsonResult<Void>> holyPost(@Valid @RequestBody HolyScheduleInfoFormRequest form, BindingResult bindingResult) {
		if (!form.validate(bindingResult))
			throw new ValidationException(bindingResult);
