...
/**
 * 음원/IVR관리 > 일정관리 > [발신]일별스케쥴러
 */
@Slf4j
@RequiredArgsConstructor
@RestController
@RequestMapping(value = "api/v1/admin/sounds/schedule/outbound/day", produces = MediaType.APPLICATION_JSON_VALUE)
public class OutboundDayScheduleSeedApiController extends ApiBaseController {
	...
	private final OutScheduleListRepository outScheduleListRepository;
	 ...
	 */
	@PostMapping("")
	public ResponseEntity<JsonResult<Void>> post(@Valid @RequestBody DayOutScheduleSeedFormRequest form, BindingResult bindingResult) {
		if (!form.validate(bindingResult))
			throw new ValidationException(bindingResult);

		repository.insertOnDayScheduleAllPbxServers(form);
		return ResponseEntity.created(URI.create("api/v1/admin/sounds/schedule/outbound/day")).body(create());
	 ...
	 */
	@PutMapping("{parent}")
	public ResponseEntity<JsonResult<Void>> update(@Valid @RequestBody DayOutScheduleSeedFormRequest form, BindingResult bindingResult, @PathVariable Integer parent) {
		if (!form.validate(bindingResult))
			throw new ValidationException(bindingResult);

		repository.updateOnDayScheduleAllPbxServers(form, parent);
	 ...
	 */
	@PostMapping("holy")
	public ResponseEntity<JsonResult<Void>> holyPost(@Valid @RequestBody HolyOutScheduleFormRequest form, BindingResult bindingResult) {
		if (!form.validate(bindingResult))
			throw new ValidationException(bindingResult);

		outScheduleListRepository.insertOnDayHolySchedule(form);
		return ResponseEntity.created(URI.create("api/v1/admin/schedule/outbound/day/holy")).body(create());
	}
