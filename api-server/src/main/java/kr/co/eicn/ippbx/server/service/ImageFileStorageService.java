...
public class ImageFileStorageService extends FileSystemStorageService {
    public String uploadImage(Path path, MultipartFile image) {
        ...
        final String saveFileName = new SimpleDateFormat("yyyyMMddHHmmssSSS").format(new Date()).concat("_") + UrlUtils.decode(cleanPath(Objects.requireNonNull(image.getOriginalFilename()))).replaceAll("[ ()]", "");
