...
@RestController
@RequestMapping(value = "api/v1/admin/wtalk/schedule/week", produces = MediaType.APPLICATION_JSON_VALUE)
public class WeekWtalkScheduleInfoApiController extends ApiBaseController {
	...
	public ResponseEntity<JsonResult<Void>> post(@Valid @RequestBody TalkScheduleInfoFormRequest form, BindingResult bindingResult) {
		...
		return ResponseEntity.created(URI.create("api/v1/admin/wtalk/schedule/week")).body(create());
	...
	public ResponseEntity<JsonResult<List<SummaryWtalkServiceResponse>>> talkServices() {
		return ResponseEntity.ok(data(talkServiceInfoRepository.findAll().stream().map(e -> convertDto(e, SummaryWtalkServiceResponse.class)).collect(Collectors.toList())));
	}
}
