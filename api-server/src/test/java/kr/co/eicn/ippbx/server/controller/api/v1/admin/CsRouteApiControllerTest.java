...
@Slf4j
public class CsRouteApiControllerTest extends BaseControllerTest {
    private final String TEST_URL = "/api/v1/admin/acd/route/cs";

    private FieldDescriptor[] csRouteApiSummaryResponse = new FieldDescriptor[]{
            fieldWithPath("seq").type(JsonFieldType.NUMBER).description("SEQUENCE KEY"),
            fieldWithPath("huntNumber").type(JsonFieldType.STRING).description("Hunt번호"),
            fieldWithPath("huntName").type(JsonFieldType.STRING).description("Hunt한글명"),
            fieldWithPath("cycle").type(JsonFieldType.STRING).description("검색주기(DAY:1일, WEEK:1주일, MONTH:1달)")
    };

    private FieldDescriptor[] csRouteApiGetResponse = new FieldDescriptor[]{
            fieldWithPath("seq").type(JsonFieldType.NUMBER).description("SEQUENCE KEY"),
            fieldWithPath("huntNumber").type(JsonFieldType.STRING).description("Hunt번호"),
            fieldWithPath("cycle").type(JsonFieldType.STRING).description("검색주기(DAY:1일, WEEK:1주일, MONTH:1달)")
    };

    private FieldDescriptor[] csRouteFormResponse = new FieldDescriptor[]{
            fieldWithPath("huntNumber").type(JsonFieldType.STRING).description("Hunt번호"),
            fieldWithPath("cycle").type(JsonFieldType.STRING).description("검색주기(DAY:1일, WEEK:1주일, MONTH:1달)")
    };
        ...
        this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL)
                ...
                .andDo(document.document(
                        requestParameters(
                                ...
                                parameterWithName("huntNumber").description("HUNT번호").optional()
                        ),
                        responseFields(
                                fieldWithPath("result").type(JsonFieldType.STRING).description("응답결과"),
                                fieldWithPath("reason").type(JsonFieldType.STRING).description("실패 메시지").optional()
                        )
                        .andWithPrefix("data.rows[]", csRouteApiSummaryResponse)
                        .andWithPrefix("data.",
    ...
    public void post() throws Exception {
        final CsRouteFormRequest form = new CsRouteFormRequest();

        form.setQueueNumber("99999999999");
        form.setCycle("MONTH");
        ...
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.post(TEST_URL)
                ...
                .andDo(document.document(
                        requestFields(csRouteFormResponse),
                        responseFields(
                                fieldWithPath("result").type(JsonFieldType.STRING).description("응답결과"),
                                fieldWithPath("reason").type(JsonFieldType.STRING).description("실패 메시지").optional(),
