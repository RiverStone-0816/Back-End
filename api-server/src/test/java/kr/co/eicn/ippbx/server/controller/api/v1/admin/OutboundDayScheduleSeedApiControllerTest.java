...
public class OutboundDayScheduleSeedApiControllerTest extends BaseControllerTest {
	...
	private FieldDescriptor[] dayOutScheduleSeedFormRequest = new FieldDescriptor[] {
			fieldWithPath("name").type(JsonFieldType.STRING).description("일정명"),
			fieldWithPath("extensions").type(JsonFieldType.ARRAY).description("추가 내선번호"),
			fieldWithPath("fromDate").type(JsonFieldType.STRING).description("발신 스케쥴러 시작일 (YYYY-MM-DD)"),
			fieldWithPath("toDate").type(JsonFieldType.STRING).description("발신 스케쥴러 종료일 (YYYY-MM-DD)"),
			fieldWithPath("fromhour").type(JsonFieldType.NUMBER).description("시작시간 분 (09:00 -> 540)"),
			fieldWithPath("tohour").type(JsonFieldType.NUMBER).description("종료시간 분 (18:00 -> 1080)"),
	...
	public void list() throws Exception {
		...
		for (OutScheduleSeedEntity response : outScheduleSeedEntities) {
			...
			for (OutScheduleListEntity outScheduleList : response.getOutScheduleLists()) {
				log.info("--> 번호({}), 날짜({}), 시간({} ~ {}), 음원명({})"
					, outScheduleList.getExtension()
					, outScheduleList.getFromtime()
...
//	@Test
	public void get() throws Exception {
		final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL + "/{parent}", 13)
				.accept(MediaType.APPLICATION_JSON)
				.with(new JwtRequestPostProcessor()))
				.andDo(print())
				.andExpect(status().isOk())
...
//	@Test
	...
	protected void post() throws Exception {
		final DayOutScheduleSeedFormRequest form = new DayOutScheduleSeedFormRequest();
		form.setName("발신 스케쥴러 테스트");
		...
		final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.post(TEST_URL)
				...
				.andDo(document.document(
						requestFields(dayOutScheduleSeedFormRequest),
						responseFields(
								fieldWithPath("result").type(JsonFieldType.STRING).description("응답결과"),
								fieldWithPath("reason").type(JsonFieldType.STRING).description("실패 메시지").optional(),
...
//	@Test
	...
	protected void delete() throws Exception {
		this.mockMvc.perform(RestDocumentationRequestBuilders.delete(TEST_URL + "/{parent}", 18)
				.with(new JwtRequestPostProcessor()))
				.andExpect(status().isOk())
...
//	@Test
	...
	public void update() throws Exception {
		...
		form.setFromDate("2020-02-28");
		form.setToDate("2020-03-04");

		final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.put(TEST_URL + "/{parent}", 17)
				.contentType(MediaType.APPLICATION_JSON)
				.with(new JwtRequestPostProcessor())
				.content(mapper.writeValueAsString(form)))
				.andDo(print())
