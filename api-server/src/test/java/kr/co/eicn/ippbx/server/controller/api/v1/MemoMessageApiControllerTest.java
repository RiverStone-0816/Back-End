...
//    @Test
    protected void list() throws Exception {
        MemoMsgSearchRequest search = new MemoMsgSearchRequest();
        search.setContent("ㅎㅎ");
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL + "/{seq}", 11)
...
//    @Test
    protected void post() throws Exception {
        final MemoMsgFormRequest form = new MemoMsgFormRequest();
...
//        memoSequences.add(1);
//        members.add("user617");
        form.setReceiveUserIds(members);
        form.setContent("테스트11");
...
//    @Test
    protected void delete() throws Exception {
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.delete(TEST_URL)
                .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)
//                .param("newRoomName", "단체")
//                .content(mapper.writeValueAsString(memoSequences))
                .with(new JwtRequestPostProcessor())
                .param("memoSequences", "2")
                .param("memoSequences", "5"))
                .andExpect(status().isOk())
//                .andDo(document.document(
//                        responseFields(
//                                fieldWithPath("result").type(JsonFieldType.STRING).description("응답결과"),
