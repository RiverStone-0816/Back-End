...
import kr.co.eicn.ippbx.server.model.form.VipRouteFormRequest;
...
@Slf4j
public class VipRouteApiControllerTest extends BaseControllerTest {
    private final String TEST_URL = "/api/v1/admin/acd/grade/viplist";

    private FieldDescriptor[] vipRouteApiSummaryResponse = new FieldDescriptor[]{
            fieldWithPath("seq").type(JsonFieldType.NUMBER).description("SEQUENCE KEY"),
            fieldWithPath("vipNumber").type(JsonFieldType.STRING).description("VIP번호"),
            fieldWithPath("type").type(JsonFieldType.STRING).description("라우트타입(A:우선분배, B:헌트분배)"),
    ...
    private FieldDescriptor[] vipRouteFormRequest = new FieldDescriptor[]{
            fieldWithPath("vipNumber").type(JsonFieldType.STRING).description("VIP번호"),
            fieldWithPath("type").type(JsonFieldType.STRING).description("라우트타입(A:우선분배, B:헌트분배)"),
        ...
        this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL)
                ...
                .andDo(document.document(
                        requestParameters(
                                ...
                                parameterWithName("vipNumber").description("VIP번호").optional()
                        ),
                        responseFields(
                                fieldWithPath("result").type(JsonFieldType.STRING).description("응답결과"),
                                fieldWithPath("reason").type(JsonFieldType.STRING).description("실패 메시지").optional()
                        )
                        .andWithPrefix("data.rows[]", vipRouteApiSummaryResponse)
                        .andWithPrefix("data.",
...
//    @Test
    ...
    public void get() throws Exception {
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL + "/{seq}", 22)
                .accept(MediaType.APPLICATION_JSON)
                .with(new JwtRequestPostProcessor()))
                .andDo(print())
                .andExpect(status().isOk())
                .andDo(document.document(
                        ...
                        responseFields(
                                ...
                                .andWithPrefix("data.", vipRouteApiSummaryResponse)
                ))
                .andReturn();
    }
    ...
    public void post() throws Exception {
        final VipRouteFormRequest form = new VipRouteFormRequest();

        form.setVipNumber("11111111222");
        form.setType("A");
        form.setHuntNumber("222222222333");
        ...
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.post(TEST_URL)
                ...
                .andDo(document.document(
                        requestFields(vipRouteFormRequest),
                        responseFields(
                                fieldWithPath("result").type(JsonFieldType.STRING).description("응답결과"),
                                fieldWithPath("reason").type(JsonFieldType.STRING).description("실패 메시지").optional(),
    ...
    protected void put() throws Exception {
        ...
        form.setVipNumber("07075490108");
        form.setType("B");
        form.setHuntNumber("01000001111");

        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.put(TEST_URL + "/{seq}", 12)
    ...
    public void delete() throws Exception {
        this.mockMvc.perform(RestDocumentationRequestBuilders.delete(TEST_URL + "/{seq}", 12)
                .with(new JwtRequestPostProcessor()))
                .andExpect(status().isOk())
