...
@Slf4j
public class DayTalkScheduleInfoApiControllerTest extends BaseControllerTest {
	private final String TEST_URL = "/api/v1/admin/talk/schedule/day";
	...
	private FieldDescriptor[] talkServiceInfoEntity = new FieldDescriptor[] {
	...
	private FieldDescriptor[] talkScheduleInfoDetailResponse = new FieldDescriptor[] {
			...
			fieldWithPath("week").type(JsonFieldType.STRING).description("요일(1Mon:월요일, 2Tue:화요일, 3Wed:수요일, 4Thu:목요일, 5Fri:금요일, 6Sat:토요일, 7Sun:일요일)"),
	...
	private FieldDescriptor[] talkScheduleInfoEntity = new FieldDescriptor[] {
			...
			fieldWithPath("fromdate").type(JsonFieldType.STRING).description("시작일"),
			fieldWithPath("todate").type(JsonFieldType.STRING).description("종료일"),
			fieldWithPath("groupCode").type(JsonFieldType.STRING).description("같은 group_level 에서 unique한 코드 4자리 ex>0001"),
			fieldWithPath("groupTreeName").type(JsonFieldType.STRING).description("윗레벨의 코드를 포함한 코드의 나열 ex>0003_0008_0001"),
			fieldWithPath("groupLevel").type(JsonFieldType.NUMBER).description("해당조직의 레벨 MAX 보다 같거나 작을것"),
			fieldWithPath("groupId").type(JsonFieldType.NUMBER).description("유형아이디"),
			fieldWithPath("companyId").type(JsonFieldType.STRING).description("고객사 아이디"),
			fieldWithPath("isStat").type(JsonFieldType.STRING).description("통계 사용여부"),
			fieldWithPath("scheduleGroup").type(JsonFieldType.OBJECT).description("상담톡유형정보").optional(),
	...
	private FieldDescriptor[] dayTalkScheduleInfoFormRequest = new FieldDescriptor[] {
...
//	@Test
	public void get_type() throws Exception {
		final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL + "/service/type/{parent}", 9)
				.accept(MediaType.APPLICATION_JSON)
				.with(new JwtRequestPostProcessor()))
				.andDo(print())
				.andExpect(status().isOk())
...
//	@Test
	public void get() throws Exception {
		final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL + "/{seq}", 98)
				.accept(MediaType.APPLICATION_JSON)
				.with(new JwtRequestPostProcessor()))
				.andDo(print())
				.andExpect(status().isOk())
...
//	@Test
	...
	protected void post() throws Exception {
		final DayTalkScheduleInfoFormRequest form = new DayTalkScheduleInfoFormRequest();
		form.setGroupId(9);
		...
		form.setFromDate("2020-02-24");
		form.setToDate("2020-02-24");
		...
		final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.post(TEST_URL)
				...
				.andDo(document.document(
						requestFields(dayTalkScheduleInfoFormRequest),
						responseFields(
								fieldWithPath("result").type(JsonFieldType.STRING).description("응답결과"),
								fieldWithPath("reason").type(JsonFieldType.STRING).description("실패 메시지").optional(),
...
//	@Test
	...
	protected void delete() throws Exception {
		this.mockMvc.perform(RestDocumentationRequestBuilders.delete(TEST_URL + "/{senderKey}", "28ee2687812321c674b2453cbd62c121bea349d4")
				.with(new JwtRequestPostProcessor()))
				.andExpect(status().isOk())
...
//	@Test
	...
	public void update_type() throws Exception {
		...
		final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.put(TEST_URL + "/service/type/{seq}", 142)
				.contentType(MediaType.APPLICATION_JSON)
				.with(new JwtRequestPostProcessor())
				.content(mapper.writeValueAsString(form)))
				.andDo(print())
...
//	@Test
	...
	protected void delete_type() throws Exception {
		final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.delete(TEST_URL + "/service/type/{seq}", 142)
