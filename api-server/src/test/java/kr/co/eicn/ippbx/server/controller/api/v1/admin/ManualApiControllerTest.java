...
@Slf4j
public class ManualApiControllerTest extends BaseControllerTest {
    private final String TEST_URL = "/api/v1/admin/help/manual";
    private static Long id;

    @Autowired
    private StorageService fileStorageSystemService;

    private FieldDescriptor[] manualSummaryResponse = new FieldDescriptor[] {
    ...
    private FieldDescriptor[] manualDetailResponse = new FieldDescriptor[] {
            fieldWithPath("id").type(JsonFieldType.NUMBER).description("글번호"),
            fieldWithPath("title").type(JsonFieldType.STRING).description("제목"),
            fieldWithPath("createdAt").type(JsonFieldType.STRING).description("등록일"),
            fieldWithPath("writer").type(JsonFieldType.STRING).description("작성자"),
        ...
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL)
                ...
                .andDo(document.document(
                        ...
                        responseFields(
                                ...
                                .andWithPrefix("data.rows[]", manualSummaryResponse)
                                .andWithPrefix("data.",
        ...
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL + "/{id}", 1)
                ...
                .andDo(document.document(
                        ...
                        responseFields(
                                ...
                                .andWithPrefix("data.", manualDetailResponse)
...
//        Resource resource = fileStorageSystemService.loadAsResource("D:/", "dropdown.hwp");
//        MockMultipartFile file = new MockMultipartFile("files", "dropdown.hwp", "", org.apache.commons.io.IOUtils.toByteArray(resource.getURI()));
//        resource = fileStorageSystemService.loadAsResource("D:/", "1.xlsx");
//        MockMultipartFile secondFile = new MockMultipartFile("files", "1.xlsx", "",  org.apache.commons.io.IOUtils.toByteArray(resource.getURI()));


        this.mockMvc.perform(MockMvcRequestBuilders.multipart(TEST_URL)
                .with(new JwtRequestPostProcessor())
                .param("title", "title"))
                .andDo(print())
...
//    @Test
    ...
    protected void put() throws Exception {
        Resource resource = fileStorageSystemService.loadAsResource("D:/", "dropdown.hwp");
        MockMultipartFile firstFile = new MockMultipartFile("files", "dropdown.hwp", "",  org.apache.commons.io.IOUtils.toByteArray(resource.getURI()));
//        resource = fileStorageSystemService.loadAsResource("D:/", "1.xlsx");
        MockMultipartFile secondFile = new MockMultipartFile("files", "1.xlsx", "",  org.apache.commons.io.IOUtils.toByteArray(resource.getURI()));

        final MvcResult result = this.mockMvc.perform(fileUpload(TEST_URL + "/{id}", 4).file(firstFile)
                .with(new JwtRequestPostProcessor())
                .param("title", "title수정"))
                .andDo(print())
                .andExpect(status().isOk())
...
//    @Test
    ...
    protected void delete() throws Exception {
        this.mockMvc.perform(RestDocumentationRequestBuilders.delete(TEST_URL + "/{id}", 3)
                .with(new JwtRequestPostProcessor()))
                .andExpect(status().isOk())
...
//  @Test
    protected void deleteSpecificFile() throws Exception {
        this.mockMvc.perform(RestDocumentationRequestBuilders.delete(TEST_URL + "/delete-specific-file/{id}", 1)
                .with(new JwtRequestPostProcessor()))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("result").value("success"))
                .andDo(document.document(
