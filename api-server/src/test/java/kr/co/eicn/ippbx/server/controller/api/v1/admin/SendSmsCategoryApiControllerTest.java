...
@Slf4j
public class SendSmsCategoryApiControllerTest extends BaseControllerTest {
    private final String TEST_URL = "/api/v1/admin/application/sms/category";
    ...
    protected void pagination() throws Exception {
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL)
                ...
                .andDo(document.document(
                        ...
                        responseFields(
                                fieldWithPath("data.rows[]").description("SMS카테고리").optional(),
...
//      @Test
    ...
    public void get() throws Exception {
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.get(TEST_URL + "/{categoryCode}", "S001")
                .accept(MediaType.APPLICATION_JSON)
                .with(new JwtRequestPostProcessor()))
                .andDo(print())
                .andExpect(status().isOk())
...
//      @Test
    ...
    protected void post() throws Exception {
        final SendSmsCategoryFormRequest form = new SendSmsCategoryFormRequest();
        form.setCategoryCode("S003");        //카테고리 코드
        form.setCategoryName("tes");   //카테고리명
        form.setCategoryType(SendCategoryType.SMS.getCode());    //카테고리 분류(S:SMS)
        ...
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.post(TEST_URL)
                ...
                .andDo(document.document(
                        requestFields(
                                fieldWithPath("categoryCode").type(JsonFieldType.STRING).description("카테고리 코드"),
                                fieldWithPath("categoryName").type(JsonFieldType.STRING).description("카테고리명"),
                                fieldWithPath("categoryType").type(JsonFieldType.STRING).description("카테고리 분류(S:SMS)")
                        ),
                        responseFields(
                                fieldWithPath("result").type(JsonFieldType.STRING).description("응답결과"),
                                fieldWithPath("reason").type(JsonFieldType.STRING).description("실패 메시지").optional(),
...
//      @Test
    ...
    protected void put() throws Exception {
        ...
        final MvcResult result = this.mockMvc.perform(RestDocumentationRequestBuilders.put(TEST_URL + "/{categoryCode}", "S003")
                .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(form))
                .with(new JwtRequestPostProcessor()))
                .andDo(print())
...
//      @Test
    ...
    protected void delete() throws Exception {
        this.mockMvc.perform(RestDocumentationRequestBuilders.delete(TEST_URL + "/{categoryCode}", "S003")
                .with(new JwtRequestPostProcessor()))
                .andDo(print())
                .andExpect(status().isOk())
