package kr.co.eicn.ippbx.model.entity.eicn;

import kr.co.eicn.ippbx.model.LicenseInfo;
import lombok.Data;
import lombok.ToString;

@ToString
@Data
public class CompanyLicenceEntity {
    private LicenseInfo recordLicense;		// 녹취 라이센스 정보
    private LicenseInfo pdsLicense;			// PDS 라이센스 정보
    private LicenseInfo statLicence;		// 통계/모니터링 라이센스 정보
    private LicenseInfo ctiLicence;			// 상담원 연결여부 라이센스 정보
    private LicenseInfo talkLicense;		// 상담톡 라이센스 정보
    private LicenseInfo emailLicense;		// 이메일 라이센스 정보
    private LicenseInfo chattLicense;		// 메신저(채팅) 라이센스 정보
    private LicenseInfo sttLicense;			// TA(STT) 라이센스 정보
    private LicenseInfo softPhoneLicense;	// 소프트폰 라이선스 정보
    private LicenseInfo astinLicense;		// astin 라이선스 정보
    private LicenseInfo astoutLicense;		// astout 라이선스 정보
    private LicenseInfo aststtLicense;		// aststt 라이선스 정보
}
