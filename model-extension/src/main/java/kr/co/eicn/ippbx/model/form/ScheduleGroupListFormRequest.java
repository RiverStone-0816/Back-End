...
public class ScheduleGroupListFormRequest extends BaseForm {
    ...
    public boolean validate(BindingResult bindingResult) {
        ...
        if (isNotEmpty(kind)) {
            ...
            } else if (ScheduleKind.EXCEPTION_CONTEXT_DIRECT_NUMBER.getCode().equals(kind)) {
                if (isEmpty(kindData))
                    reject(bindingResult, "kindData", "{컨텍스트는 필수 입력사항입니다.}");

                final String[] kindDataArray = kindData.split("\\|");
                if (kindDataArray.length < 2)
                    reject(bindingResult, "kindData", "{내부번호는 필수 입력사항입니다.}");

                if (kindDataArray.length < 1)
                    reject(bindingResult, "kindData", "{컨텍스트는 필수 입력사항입니다.}");
            } else if (ScheduleKind.EXCEPTION_CONTEXT_IVR_CONNECT.getCode().equals(kind)) {
                if (isEmpty(kindData))
                    reject(bindingResult, "kindData", "{컨텍스트는 필수 입력사항입니다.}");
